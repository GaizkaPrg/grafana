...
func (moa *MultiOrgAlertmanager) gettableUserConfigFromAMConfigString(ctx context.Context, orgID int64, config string) (definitions.GettableUserConfig, error) {
	...
	for i := range cfg.ExtraConfigs {
		if cfg.ExtraConfigs[i].AlertmanagerConfig != "" {
			decryptedData, err := moa.Crypto.DecryptBase64(ctx, cfg.ExtraConfigs[i].AlertmanagerConfig, secrets.WithoutScope())
			if err != nil {
				return definitions.GettableUserConfig{}, fmt.Errorf("failed to decrypt external configuration %d: %w", i, err)
			}
			cfg.ExtraConfigs[i].AlertmanagerConfig = decryptedData
		}
	}
