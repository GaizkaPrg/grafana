...
func (am *alertmanager) SaveAndApplyConfig(ctx context.Context, cfg *apimodels.PostableUserConfig) error {
	...
	// Handle external configs: each external config is encrypted and stored as a base64 string.
	for i := range cfg.ExtraConfigs {
		encryptedCfg, err := am.crypto.EncryptBase64(ctx, cfg.ExtraConfigs[i].AlertmanagerConfig, secrets.WithoutScope())
		if err != nil {
			return fmt.Errorf("failed to encrypt external configuration: %w", err)
		}
		cfg.ExtraConfigs[i].AlertmanagerConfig = encryptedCfg
	}
	...
	am.Base.WithLock(func() {
		...
		err = am.Store.SaveAlertmanagerConfigurationWithCallback(ctx, cmd, func() error {
			// Restore the original unencrypted external configurations back
			// to the user config, so that we can apply it.
...
func (am *alertmanager) applyConfig(ctx context.Context, cfg *apimodels.PostableUserConfig, skipInvalid bool) (bool, error) {
	// Decrypt external configs
	for i := range cfg.ExtraConfigs {
		if cfg.ExtraConfigs[i].AlertmanagerConfig != "" {
			decryptedCfg, err := am.crypto.DecryptBase64(ctx, cfg.ExtraConfigs[i].AlertmanagerConfig, secrets.WithoutScope())
			if err != nil {
				return false, fmt.Errorf("failed to decrypt external configuration %d: %w", i, err)
			}
			cfg.ExtraConfigs[i].AlertmanagerConfig = decryptedCfg
		}
	}
