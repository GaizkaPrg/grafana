...
func (srv *ConvertPrometheusSrv) RouteConvertPrometheusGetAlertmanagerConfig(c *contextmodel.ReqContext) response.Response {
	...
	resp := apimodels.AlertmanagerUserConfig{}
	...
	for _, extraConfig := range cfg.ExtraConfigs {
		...
		amConfig, err := extraConfig.GetAlertmanagerConfig()
		if err != nil {
			logger.Error("Failed to get alertmanager config from extra configuration", "error", err)
			return errorToResponse(fmt.Errorf("failed to get alertmanager config: %w", err))
		}

		promConfig := alertingdef.GrafanaToUpstreamConfig(&amConfig)
